...
// --------------------------------------------------------------
  ...
  _findWallByKey(key) {
    if (!this.poolGroup || key == null) return null;
    let match = null;
    this.poolGroup.traverse((o) => {
      if (!match && o?.userData?.isWall && this._getWallRaiseKey(o) === key) {
        match = o;
      }
    });
    return match;
  ...
  _syncWallHighlightPositions() {
    if (!this.poolGroup) return;

    if (this.selectedWall) {
      const selectedKey = this._getWallRaiseKey(this.selectedWall);
      const liveSelectedWall = this._findWallByKey(selectedKey) || this.selectedWall;
      if (liveSelectedWall !== this.selectedWall) {
        this.selectedWall = liveSelectedWall;
      }
      if (this.selectedWallHighlightMesh?.visible && liveSelectedWall) {
        this.updateHighlightForWall(liveSelectedWall, true);
      }
    }

    if (this.hoveredWall) {
      const hoverKey = this._getWallRaiseKey(this.hoveredWall);
      const liveHoveredWall = this._findWallByKey(hoverKey) || this.hoveredWall;
      if (liveHoveredWall !== this.hoveredWall) {
        this.hoveredWall = liveHoveredWall;
      }
      if (this.hoverWallHighlightMesh?.visible && liveHoveredWall && liveHoveredWall !== this.selectedWall) {
        this.updateHighlightForWall(liveHoveredWall, false);
      }
    }
  }
...
// Keep freeform handles screen-aligned and interactive
    ...
    this._syncWallHighlightPositions();
