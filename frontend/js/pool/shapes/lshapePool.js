...
/* -------------------------------------------------------
  ...
  /* -------------------------------------------------------
     COPING SEGMENTS (match custom/freeform behaviour)
  ------------------------------------------------------- */
  ...
  for (let i = 0; i < borderPts.length; i++) {
    const copingGeo = createMiteredWallPrism(borderPts, i, wallThickness, copingDepth);
    if (!copingGeo) continue;

    generateMeterUVsForBoxGeometry(copingGeo, tileSize);

    const coping = new THREE.Mesh(copingGeo, copingMat.clone());
    coping.castShadow = true;
    coping.receiveShadow = true;
    coping.position.z = zOffset;
    coping.renderOrder = 3;
    coping.userData.isCoping = true;
    coping.userData.baseZ = coping.position.z;

    copingSegments.push(coping);
    group.add(coping);
  }

  group.userData.copingSegments = copingSegments;
